...
const PORT         = 4005;
const MIN_DIFF_PCT = 13;
const STABLE_SEC   = 8;
const MAX_MINUTE   = 3;
const STAKE        = 1;
const GT_DURATION  = 12; // minuta trajanje GT meča

const HISTORY_FILE = path.join(__dirname, "bets-history-4005.json");
...
// Praćeni kandidati — graduirani iz candidates, čekaju score uvjet za klađenje
// value: { matchName, admId, b365Id, outcome, admOdd, b365Odd, diff, minute,
//          trackedSince, betPlacedAt1, betPlacedAt2 }
const trackedCandidates = new Map();
...
// ── Deficit tima za dati ishod ────────────────────────────────────────────────
// Vraća koliko gubi tim koji treba pobijediti:
//   outcome "1" → domaćin gubi ako je home < away, deficit = away - home
//   outcome "2" → gost gubi ako je away < home,   deficit = home - away
// Vraća null ako score nije čitljiv, negativno/0 ako vodi ili izjednačen.
function getDeficit(score, outcome) {
  const m = String(score ?? "").match(/(\d+)\D+(\d+)/);
  if (!m) return null;
  const h = parseInt(m[1]), a = parseInt(m[2]);
  if (outcome === "1") return a - h; // pozitivno = domaćin gubi
  if (outcome === "2") return h - a; // pozitivno = gost gubi
  return null;
}
...
async function refresh() {
  ...
  for (const bs of b365slots) {
    ...
    for (const k of ["1", "2"]) {
      ...
      const scoreZeroZero = normalizeScore(bestAdmSlot.score) === "0:0";
      const isCandidate = canBet && diff >= MIN_DIFF_PCT && admOddOk && scoreZeroZero;
      probRows[k] = { pAdm, pB365, diff, isCandidate };

      if (isCandidate) {
        const key = `${bestAdmSlot.id}_${k}`;
        activeCandidateKeys.add(key);
        if (!candidates.has(key) && !trackedCandidates.has(key)) {
          candidates.set(key, {
            since: now,
            matchName: bs.name,
          ...
          // sendTelegram(`🔴🔴🔴 <b>IGRAJ</b> 🔴🔴🔴\nMeč: ${bs.name}\nIshod: <b>${k}</b> | Min: ${minute != null ? minute + "'" : "--"}\nAdmiral: ${adm1[k].toFixed(2)} | Razlika: +${diff.toFixed(1)}%`); // 4005: telegram privremeno obustavljen
        ...
        // ako je već u trackedCandidates — ne diraj
      }
    }
  ...
  // Provjeri kandidate koji su stabilni ≥ 8s → promoviši u trackedCandidates
  for (const [key, c] of candidates) {
    const elapsed = (now - c.since) / 1000;
    if (elapsed >= STABLE_SEC) {
      if (!trackedCandidates.has(key)) {
        trackedCandidates.set(key, {
          ...
          trackedSince: now,
          betPlacedAt1: false,   // bet kad gubi za 1
          betPlacedAt2: false,   // bet kad gubi za 2
          deficit1Since: null,   // kad je deficit -1 prvi put viđen
          deficit2Since: null,   // kad je deficit -2 prvi put viđen
        });
        console.log(`TRACKED: ${c.matchName} | ${c.outcome} | diff=${c.diff.toFixed(1)}% min=${c.minute} — čekam score uvjet`);
        sendTelegram(
          `⚫⚫⚫⚫⚫\n\n` +
          `👁️ <b>ČEKELICA</b> — ${c.minute != null ? c.minute + "'" : "--"}\n\n` +
          `Meč: ${c.matchName}\n` +
          `Ishod: <b>${c.outcome}</b> | Diff: +${c.diff.toFixed(1)}%\n` +
          `Admiral: ${c.admOdd.toFixed(2)} | B365: ${c.b365Odd.toFixed(2)}\n` +
          `Čekam da gubi...\n\n` +
          `⚫⚫⚫⚫⚫`
        );
      }
  ...
  // Provjeri trackedCandidates — odigraj bet kad score ispuni uvjet
  for (const [key, tc] of trackedCandidates) {
    const admSlot  = admSlots.find(s => s.id === tc.admId);
    const b365Slot = b365slots.find(s => s.id === tc.b365Id);

    // Ako meč nestao s oba feeda — završili smo praćenje
    if (!admSlot && !b365Slot) {
      trackedCandidates.delete(key);
      continue;
    }

    // Tekuća minuta (Admiral primarni, inače B365)
    const curMinute = admSlot
      ? parseMinute(admSlot.minute)
      : parseMinute(b365Slot?.time);

    // Uzmi score s oba izvora
    const admScore  = (admSlot?.score && admSlot.score !== "--")
      ? admSlot.score
      : (lastSeenScore.get(tc.admId) ?? null);
    const b365Score = (b365Slot?.score && b365Slot.score !== "--")
      ? b365Slot.score
      : (lastB365ScoreCache.get(tc.b365Id) ?? null);

    // Scoreovi moraju biti poznati i isti na oba izvora
    const admNorm  = normalizeScore(admScore);
    const b365Norm = normalizeScore(b365Score);
    if (!admNorm || !b365Norm || admNorm !== b365Norm) {
      // Score nije potvrđen — resetuj timere stabilnosti
      tc.deficit1Since = null;
      tc.deficit2Since = null;
      continue;
    }

    const deficit = getDeficit(admNorm, tc.outcome);

    // Ažuriraj timer za deficit -1
    if (deficit === 1) {
      if (tc.deficit1Since == null) tc.deficit1Since = now;
    } else {
      tc.deficit1Since = null; // deficit nestao ili promijenjen — resetuj
    }

    // Ažuriraj timer za deficit -2
    if (deficit === 2) {
      if (tc.deficit2Since == null) tc.deficit2Since = now;
    } else {
      tc.deficit2Since = null;
    }

    // Uzmi trenutne kvote — meč mora biti uparen sa B365 i Admiral mora imati 1X2
    const curAdm1  = admSlot ? admiral1x2(admSlot) : null;
    const curB365  = b365Slot ? bet3651x2(b365Slot) : null;
    const curAdmOdd  = curAdm1  ? curAdm1[tc.outcome]  : null;
    const curB365Odd = curB365  ? curB365[tc.outcome] : null;

    // Izračunaj trenutnu razliku (B365prob - Admprob)
    const curDiff = (curAdmOdd && curB365Odd)
      ? ((1 / curB365Odd) - (1 / curAdmOdd)) * 100
      : null;

    const placeBet5 = (deficitLevel) => {
      // Ako nema trenutnih kvota — čekaj sljedeći refresh
      if (!curAdmOdd || !curB365Odd) {
        console.log(`BET čeka kvotu (deficit=${deficitLevel}): ${tc.matchName} | ${tc.outcome} | min=${curMinute}`);
        return false;
      }
      bets.unshift({
        ...
        b365Id:     tc.b365Id,
        outcome:    tc.outcome,
        admOdd:     parseFloat(curAdmOdd.toFixed(2)),
        b365Odd:    parseFloat(curB365Odd.toFixed(2)),
        diff:       parseFloat((curDiff ?? 0).toFixed(1)),
        betMinute:  curMinute ?? tc.minute,
        betDeficit: deficitLevel,
        stake:      STAKE,
        status:     "pending",
        finalScore: null,
        payout:     null,
      ...
      totalStaked += STAKE;
      saveHistory();
      console.log(`BET (deficit=${deficitLevel}): ${tc.matchName} | ${tc.outcome} | score=${admNorm} min=${curMinute} adm=${curAdmOdd.toFixed(2)} diff=${(curDiff??0).toFixed(1)}%`);
      const minStr = curMinute != null ? `${curMinute}'` : "--";
      sendTelegram(
        `🟣🟣🟣🟣🟣\n\n` +
        `🎯 <b>GUBILICA IGRA</b> — ${minStr}\n\n` +
        `Meč: ${tc.matchName}\n` +
        `Ishod: <b>${tc.outcome}</b> | Zaostatak: <b>-${deficitLevel}</b>\n` +
        `Score: ${admNorm} | Minuta: ${minStr}\n` +
        `Admiral: ${curAdmOdd.toFixed(2)} | B365: ${curB365Odd.toFixed(2)} | Razl: +${(curDiff??0).toFixed(1)}%\n\n` +
        `🟣🟣🟣🟣🟣`
      );
      return true;
    };

    // Deficit -1: stabilan ≥ 8s i minuta < 10
    if (!tc.betPlacedAt1 && tc.deficit1Since != null &&
        (now - tc.deficit1Since) >= STABLE_SEC * 1000 &&
        curMinute != null && curMinute < 10) {
      if (placeBet5(1)) tc.betPlacedAt1 = true;
    }

    // Deficit -2: stabilan ≥ 8s i minuta < 6
    if (!tc.betPlacedAt2 && tc.deficit2Since != null &&
        (now - tc.deficit2Since) >= STABLE_SEC * 1000 &&
        curMinute != null && curMinute < 6) {
      if (placeBet5(2)) tc.betPlacedAt2 = true;
    }

    // Ukloni iz praćenja kad su oba prozora zatvorena
    const win1Closed = tc.betPlacedAt1 || (curMinute != null && curMinute >= 10);
    const win2Closed = tc.betPlacedAt2 || (curMinute != null && curMinute >= 6);
    if (win1Closed && win2Closed) {
      trackedCandidates.delete(key);
    }
  }
